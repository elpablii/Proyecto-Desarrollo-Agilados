...
async function cargarDatosCarrera(rut) {
    ...
    try {
        ...
        const response = await authClient.authenticatedFetch(`http://localhost:3001/carreras/${rut}`, {
            method: 'GET'
        });
        ...
        if (error.message.includes('Error de conexión')) {
            mostrarError('Error de conexión al servidor. Verifica que el backend esté ejecutándose.');
        } else if (error.message.includes('Sesión expirada')) {
            mostrarError('Tu sesión ha expirado. Por favor, inicia sesión nuevamente.');
        } else {
            mostrarError('Error al cargar datos de carrera: ' + error.message);
        }
    }
}
